...
# 32-bit fix: old CryEngine tables store hex constants > INT_MAX in 'long'
# arrays, which clang rejects as narrowing on 32-bit targets. This file is
# included after every project() call, so it survives CMakeLists overriding
# CMAKE_CXX_FLAGS.
cat > "$BUILD_DIR/compat_flags.cmake" <<'EOF'
...
cmake -B "$BUILD_DIR" -S "$ROOT_DIR" \
    ...
    -DCMAKE_PROJECT_INCLUDE="$BUILD_DIR/compat_flags.cmake"
